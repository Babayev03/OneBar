...
**Auto Mouse Move:** `MouseMoveService` sweeps the cursor out and back along an eased 60fps path of `.mouseMoved` `CGEvent`s posted at `.cghidEventTap` (a single jump would be coalesced into no visible movement, and duration is derived from distance ÷ speed so long and short sweeps look alike), which resets `HIDIdleTime` — the clock Teams/Slack read to decide you're away. In idle-aware mode the timer ticks faster than the configured interval and the nudge is gated on `CGEventSource.secondsSinceLastEventType(.hidSystemState, …)`, so real input postpones it rather than racing it. Each glide step compares the cursor's actual position against the last one posted and aborts if they diverge — our own events land exactly where we put them, so any drift is a real hand on the mouse. Needs Accessibility, mirrors `SleepPreventionManager` for auto-off and the second status item, and clicking is deliberately out of scope here — it's a separate feature.
